...
subtitle: <h1>An Application to the County of Grande Prairie and 
          Bear Lake, Alberta</h>
...
# Import data
ab_cwpt_indices <- st_read("./SpatialData/Clean/ab_cwpt_indices.shp",
                      stringsAsFactors = FALSE, quiet = TRUE) # QS data 

ab_counties <- st_read("./SpatialData/Clean/ab_counties.shp",
                      stringsAsFactors = FALSE, quiet = TRUE) # Counties 

bear_catch <- st_read("./SpatialData/Clean/BearLake_Catch/BearLake_catch_v1_LDTM.shp",
                      stringsAsFactors = FALSE, quiet = TRUE) # Bear Lake catchment

bear_cwpt <- st_read("./SpatialData/Clean/bearlakes_cwpt.shp",
                     stringsAsFactors = FALSE, quiet = TRUE) # Bear Lakes QS

# Filter for County of Grande Prairie No. 1
gp_cwpt <- ab_cwpt_indices %>%
  filter(C_MD_SA == "County of Grande Prairie No. 1")

gp <- ab_counties %>%
  filter(C_MD_SA == "County of Grande Prairie No. 1")

# Set projection
gp_cwpt <- st_transform(gp_cwpt, "+init=epsg:4326")
gp <- st_transform(gp, "+init=epsg:4326")
bear_catch <- st_transform(bear_catch, "+init=epsg:4326")
bear_cwpt <- st_transform(bear_cwpt, "+init=epsg:4326")

names(st_geometry(gp)) = NULL
names(st_geometry(gp_cwpt)) = NULL
names(st_geometry(bear_catch)) = NULL
names(st_geometry(bear_cwpt)) = NULL
...
The map below shows the landscape importance score associated with each quarter-section located within the County of Grande Prairie, Alberta. The user can select specific quarter-sections and be given the legal land description and importance score, as well as display satellite imagery. 
...
# Define colours
land <- colorNumeric("Blues", domain = gp_cwpt$land_mp) # For landscape importance index score
...
gp_cwpt$colour <- c2(10)[gp_cwpt$self_bn]
bear_cwpt$colour <- c2(10)[bear_cwpt$self_bn]
cp <- colorNumeric(c("#00441b","#238b45", "#74c476","#a1d99b", "#e5f5e0", 
                     "#f6e8c3","#dfc27d", "#bf812d","#8c510a", "#543005"),
                   domain = gp_cwpt$self_bn)
...
bear_cwpt_5 <- bear_cwpt %>%
  filter(land_mp >= 5)
bear_cwpt_7 <- bear_cwpt %>%
  filter(land_mp >= 7)
bear_cwpt_9 <- bear_cwpt %>%
  filter(land_mp >= 9)
...
gp %>%
  ...
  addResetMapButton() %>%
  addMapPane(name = "Bear Lake Catchment", zIndex = 420) %>%
  ...
  addPolygons(data = gp_cwpt, color = "#444444", weight = 0.4, smoothFactor = 0.5,
              opacity = 1.0, fillOpacity = 0.6,
              fillColor = ~ land(land_mp), group = "Landscape Position Importance Score (County)", 
              popup = paste("Legal Land Description:", gp_cwpt$LLD, "<br>",
                            "Landscape Position Importance Score:", gp_cwpt$land_mp),
              options = leafletOptions(pane = "Layers")) %>%
  
  addPolygons(data = bear_cwpt, color = "#444444", weight = 0.4, smoothFactor = 0.5,
              opacity = 1.0, fillOpacity = 0.6,
              fillColor = ~ land(land_mp), 
              group = "Landscape Position Importance Score (BL Catchment)", 
              popup = paste("Legal Land Description:", bear_cwpt$LLD, "<br>",
                            "Landscape Position Importance Score:", bear_cwpt$land_mp),
              options = leafletOptions(pane = "Layers")) %>%
  
  addPolylines(data = bear_catch, color = "#0a0909", weight = 3, smoothFactor = 0.2,
               opacity = 3.0, group = "Bear Lake Catchment Boundary", fill = FALSE,
               options = leafletOptions(pane = "Bear Lake Catchment")) %>%
  
  # Legend Layer
  addLegend(data = gp_cwpt, position = "bottomright", pal = land, values = ~ land_mp,
            title = "Index Score", opacity = 1,
            group = "Landscape Position Importance Score (County)") %>%

  # Layers Control
  addLayersControl(overlayGroups = c("Bear Lake Catchment Boundary",
                                     "Imagery"),
                   baseGroups = c("Landscape Position Importance Score (County)",
                                  "Landscape Position Importance Score (BL Catchment)",
                                  "None"),
...
gp %>%
  ...
  addPolygons(data = gp_cwpt, color = "#444444", weight = 0.4, smoothFactor = 0.5,
              opacity = 1.0, fillOpacity = 0.6,
              fillColor = ~ colour, group = "Pixel Contribution (County - All)", 
              popup = paste("Legal Land Description:", gp_cwpt$LLD, "<br>",
                            "Score:", gp_cwpt$self_bn),
              options = leafletOptions(pane = "Layers")) %>%
  
  addPolygons(data = bear_cwpt, color = "#444444", weight = 0.4, smoothFactor = 0.5,
              opacity = 1.0, fillOpacity = 0.6,
              fillColor = ~ colour, group = "Pixel Contribution (BL - All)", 
              popup = paste("Legal Land Description:", bear_cwpt$LLD, "<br>",
                            "Score:", bear_cwpt$self_bn),
              options = leafletOptions(pane = "Layers")) %>%
  
  addPolygons(data = bear_cwpt_5, color = "#444444", weight = 0.4, smoothFactor = 0.5,
              opacity = 1.0, fillOpacity = 0.6,
              fillColor = ~ colour, group = "Pixel Contribution (BL - Landscape >= 5)", 
              popup = paste("Legal Land Description:", bear_cwpt_5$LLD, "<br>",
                            "Score:", bear_cwpt_5$self_bn),
              options = leafletOptions(pane = "Layers")) %>%
  
  addPolygons(data = bear_cwpt_7, color = "#444444", weight = 0.4, smoothFactor = 0.5,
              opacity = 1.0, fillOpacity = 0.6,
              fillColor = ~ colour, group = "Pixel Contribution (BL - Landscape >= 7)", 
              popup = paste("Legal Land Description:", bear_cwpt_7$LLD, "<br>",
                            "Score:", bear_cwpt_7$self_bn),
              options = leafletOptions(pane = "Layers")) %>%
  
  addPolygons(data = bear_cwpt_9, color = "#444444", weight = 0.4, smoothFactor = 0.5,
              opacity = 1.0, fillOpacity = 0.6,
              fillColor = ~ colour, group = "Pixel Contribution (BL - Landscape >= 9)", 
              popup = paste("Legal Land Description:", bear_cwpt_9$LLD, "<br>",
                            "Score:", bear_cwpt_9$self_bn),
              options = leafletOptions(pane = "Layers")) %>%
  ...
  # Legend Layers
  addLegend(data = gp_cwpt, position = "bottomright", pal = cp, values = ~ self_bn,
            title = "Pixel Contribution Score", opacity = 1) %>%

  # Layers Control
  addLayersControl(overlayGroups = "Imagery",
                   baseGroups = c("None",
                                  "Pixel Contribution (County - All)",
                                  "Pixel Contribution (BL - All)", 
                                  "Pixel Contribution (BL - Landscape >= 5)",
                                  "Pixel Contribution (BL - Landscape >= 7)", 
                                  "Pixel Contribution (BL - Landscape >= 9)"),
                   options = layersControlOptions(collapsed = FALSE)) %>%
  hideGroup(c("Pixel Contribution (County - All)",
              "Pixel Contribution (BL - All)", 
              "Pixel Contribution (BL - Landscape >= 5)",
              "Pixel Contribution (BL - Landscape >= 7)", 
              "Pixel Contribution (BL - Landscape >= 9)",
