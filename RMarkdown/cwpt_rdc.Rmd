...
subtitle: <h1>An Application to Red Deer County, Alberta</h>
...
The non-point source runoff of nutrients across the landscape is estimated using export coefficients developed for major landcover and human footprint types, in conjunction with annual precipitation and simulation of overland flow based on a hydrologically corrected digital elevation model [(Habib et al 2016)](https://peerj.com/articles/2814/). Speficially, the CWPT relies on three modelled attributes of the landscape to evaluate the relative importance of each quarter-section:
...
The map below shows the landscape importance score associated with each quarter-section located within Red Deer County, Alberta. The user can select specific quarter-sections and be given the legal land description and importance score, as well as display satellite imagery.  
...
rdc_cwpt <- ab_cwpt_indices %>%
  filter(C_MD_SA == "Red Deer County")

rdc <- ab_counties %>%
  filter(C_MD_SA == "Red Deer County")

# Set projection
rdc_cwpt <- st_transform(rdc_cwpt, "+init=epsg:4326")
rdc <- st_transform(rdc, "+init=epsg:4326")

# Define colours
land <- colorNumeric("Blues", domain = rdc_cwpt$land_mp) # For landscape importance index score
...
rdc %>%
  ...
  addPolygons(color = "#282828", weight = 2, smoothFactor = 0.2,
              opacity = 2.0) %>%
  addPolygons(data = rdc_cwpt, color = "#444444", weight = 0.4, smoothFactor = 0.5,
              opacity = 1.0, fillOpacity = 0.6,
              fillColor = ~ land(land_mp), group = "Landscape Position Importance Score", 
              popup = paste("Legal Land Description:", rdc_cwpt$LLD, "<br>",
                            "Landscape Position Importance Score:", rdc_cwpt$land_mp)) %>%
  
  # Legend Layer
  addLegend(data = rdc_cwpt, position = "bottomright", pal = land, values = ~ land_mp,
...
rdc %>%
  ...
  addPolygons(data = rdc_cwpt, color = "#444444", weight = 0.4, smoothFactor = 0.5,
              ...
              popup = paste("Legal Land Description:", rdc_cwpt$LLD, "<br>",
                            "Score:", rdc_cwpt$self_bn)) %>%
  addPolygons(data = rdc_cwpt_5, color = "#444444", weight = 0.4, smoothFactor = 0.5,
              opacity = 1.0, fillOpacity = 0.6,
              fillColor = ~ colour, group = "On-Pixel Contribution (Landscape >= 5)", 
              popup = paste("Legal Land Description:", rdc_cwpt_5$LLD, "<br>",
                            "Score:", rdc_cwpt_5$self_bn)) %>%
  addPolygons(data = rdc_cwpt_7, color = "#444444", weight = 0.4, smoothFactor = 0.5,
              opacity = 1.0, fillOpacity = 0.6,
              fillColor = ~ colour, group = "On-Pixel Contribution (Landscape >= 7)", 
              popup = paste("Legal Land Description:", rdc_cwpt_7$LLD, "<br>",
                            "Score:", rdc_cwpt_7$self_bn)) %>%
  addPolygons(data = rdc_cwpt_9, color = "#444444", weight = 0.4, smoothFactor = 0.5,
              opacity = 1.0, fillOpacity = 0.6,
              fillColor = ~ colour, group = "On-Pixel Contribution (Landscape >= 9)", 
              popup = paste("Legal Land Description:", rdc_cwpt_9$LLD, "<br>",
                            "Score:", rdc_cwpt_9$self_bn)) %>%
  
  # Legend Layers
  addLegend(data = rdc_cwpt, position = "bottomright", pal = cp, values = ~ self_bn,
