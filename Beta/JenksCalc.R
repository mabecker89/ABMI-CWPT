...
write.csv(all_umax, "./Beta/StagingData/all_umax_j1.csv")
write.csv(all_umea, "./Beta/StagingData/all_umea_j1.csv")
write.csv(all_dmean, "./Beta/StagingData/all_dmean_j1.csv")
write.csv(all_pix, "./Beta/StagingData/all_pix_j1.csv")
write.csv(all_nl, "./Beta/StagingData/all_nl_j1.csv")
write.csv(all_wain, "./Beta/StagingData/all_wain_j1.csv")
...
# HUC2 level - So I actually probably want to take out the green zone quarters 
# for these calculations. 

dmean_2 <- ab_cwpt2_qs_3_2 %>%
  # Filter out high-end outliers - to be assigned a '10' later
  filter(pt_dmean_2 <= 0.99,
         GWA_NAME == "White Area") %>%
  split(.$HUC_2) %>%
  map(~ classIntervals(.$down_15__2, style = "fisher", n = 10))

# Pakowki Lake - Milk River - 02
plmr_dmean <- ab_cwpt2_qs_3_2 %>%
  filter(HUC_2 == "02",
         pt_dmean_2 <= 0.99,
         GWA_NAME == "White Area") %>%
  select(LLD, down_15__2) %>%
  mutate(j_dmean_2 = cut(down_15__2, 
                         breaks = dmean_2[["02"]]$brks,
                         labels = as.numeric(1:10))) %>%
  mutate(j_dmean_2 = replace_na(j_dmean_2, 1))

# South Saskatchewan River - 04
ssr_dmean <- ab_cwpt2_qs_3_2 %>%
  filter(HUC_2 == "04",
         pt_dmean_2 <= 0.99,
         GWA_NAME == "White Area") %>%
  select(LLD, down_15__2) %>%
  mutate(j_dmean_2 = cut(down_15__2, 
                         breaks = dmean_2[["04"]]$brks,
                         labels = as.numeric(1:10))) %>%
  mutate(j_dmean_2 = replace_na(j_dmean_2, 1))

# Sounding Creek - 07
sc_dmean <- ab_cwpt2_qs_3_2 %>%
  filter(HUC_2 == "07",
         pt_dmean_2 <= 0.99,
         GWA_NAME == "White Area") %>%
  select(LLD, down_15__2) %>%
  mutate(j_dmean_2 = cut(down_15__2, 
                         breaks = dmean_2[["07"]]$brks,
                         labels = as.numeric(1:10))) %>%
  mutate(j_dmean_2 = replace_na(j_dmean_2, 1))

# Red Deer River - 08
rdr_dmean <- ab_cwpt2_qs_3_2 %>%
  filter(HUC_2 == "08",
         pt_dmean_2 <= 0.99,
         GWA_NAME == "White Area") %>%
  select(LLD, down_15__2) %>%
  mutate(j_dmean_2 = cut(down_15__2, 
                         breaks = dmean_2[["08"]]$brks,
                         labels = as.numeric(1:10))) %>%
  mutate(j_dmean_2 = replace_na(j_dmean_2, 1))
...
br_dmean <- ab_cwpt2_qs_3_2 %>%
  ...
  select(LLD, down_15__2) %>%
  mutate(j_dmean_2 = cut(down_15__2, 
                         breaks = dmean_2[["12"]]$brks,
                         labels = as.numeric(1:10))) %>%
  mutate(j_dmean_2 = replace_na(j_dmean_2, 1))

# Athabasca River Basin - 17
arb_dmean <- ab_cwpt2_qs_3_2 %>%
  filter(HUC_2 == "17",
         pt_dmean_2 <= 0.99,
         GWA_NAME == "White Area") %>%
  select(LLD, down_15__2) %>%
  mutate(j_dmean_2 = cut(down_15__2, 
                         breaks = dmean_2[["17"]]$brks,
                         labels = as.numeric(1:10))) %>%
  mutate(j_dmean_2 = replace_na(j_dmean_2, 1))
